...
public class OniGen : MonoBehaviour
{
    //Chaisor's Prefab
    public GameObject NPCPrefab;

    public int popinterval = 10;//出現する日にち
    private int time = 0;
    private int poplim = 0;
    private int day = 0;
    ...
    void CountMethod()
    {
        ...
        if (time % popinterval == 0 && day + grm.evilper / 50 - 1 > poplim)//2日目以降に出現 悪行割合50%以上で1人発生
        {
            poplim++;
        }
        else if (time % popinterval == 0)//popinterval
        {
            while (day + grm.evilper / 50 - 1 < poplim && poplim > 0) //悪行の割合に応じて減らす
            {
                poplist.Remove(poplist[poplim - 1]);
                Destroy(poplist[poplim - 1]);
                poplim--;
                if (poplim < 0)
                {
                    poplim = 0;
