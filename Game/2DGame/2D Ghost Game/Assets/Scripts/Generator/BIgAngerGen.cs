...
public class BIgAngerGen : MonoBehaviour
{
    //Chaisor's Prefab
    ...
    void CountMethod()
    {
        ...
        if (time % popinterval == 0 && day + grm.goodper / 50 - 1 > poplim)//2日目以降に出現 善行割合50%以上で1人発生
        {
            poplim++;
        }
        else if (time % popinterval == 0)
        {
            while (day + grm.goodper / 50 - 1 < poplim && poplim > 0)
            {
